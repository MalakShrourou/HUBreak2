...
<body>
    <div class="main">
        ...
        <div class="calculate">
            ...
            if (!$database = mysqli_connect("localhost", "root", "12345678", "hubreak2_db"))
                die("Sorry, could not connect to the server.");
            extract($_POST);
            $query = "select orders.ID,price,name from Orders,products where orders.productId=products.ID and resturantID=4 and frompos=1 and payed=0";
            $result = mysqli_query($database, $query);
    ...
    <div class="inv">
        <?php
        $qq2 = "select ID from carts where resturantId=4 and customerID != 8";
        $result = mysqli_query($database, $qq2);
        while ($row = mysqli_fetch_row($result)) {
            foreach ($row as $value) {
                $Rid = $value;
                print("<a href='bill_village.php'>رقم الطلب : $value</a><br><br>");
            }
        }
        ?>
    </div>
