...
<body>
    <div class="main">
        <div class="section">
            ...
            extract($_POST);
            $query = "select products.ID,products.name,resturantproducts.price,resturantproducts.resturantId from products join resturantproducts on products.ID = productId WHERE resturantproducts.resturantId = 2 ";
            $result = mysqli_query($database, $query);
            while ($row = mysqli_fetch_row($result)) {
                print("<form method='post' action='addtocart.php'>");
                print("<input type='hidden' name='link' value='Espresso.php'>");
                $x = 0;
        ...
        <div class="calculate">
            ...
            if (!$database = mysqli_connect("localhost", "root", "12345678", "hubreak2_db"))
                die("Sorry, could not connect to the server.");
            extract($_POST);
            $query = "select orders.ID,price,name from Orders,products where orders.productId=products.ID and resturantID=2";
            $result = mysqli_query($database, $query);
            ...
            mysqli_close($database);
            ?>
            <input type="hidden" name="RestId" value=2>
