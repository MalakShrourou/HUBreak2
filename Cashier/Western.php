...
<body>
    <div class="main">
        <div class="section">
            ...
            extract($_POST);
            $query = "select products.ID,products.name,resturantproducts.price,resturantproducts.resturantId from products join resturantproducts on products.ID = productId WHERE resturantproducts.resturantId = 6 ";
            $result = mysqli_query($database, $query);
            while ($row = mysqli_fetch_row($result)) {
                print("<form method='post' action='addtocart.php'>");
                print("<input type='hidden' name='link' value='Western.php'>");
                $x = 0;
        ...
        <div class="calculate">
            ...
            if (!$database = mysqli_connect("localhost", "root", "12345678", "hubreak2_db"))
                die("Sorry, could not connect to the server.");
            extract($_POST);
            $query = "select orders.ID,price,name from Orders,products where orders.productId=products.ID and resturantID=6 and frompos=1 and payed=0";
            $result = mysqli_query($database, $query);
    ...
    <div class="inv">
        <?php
        $qq2 = "select ID from carts where resturantId=6 and customerID != 8";
        $result = mysqli_query($database, $qq2);
        while ($row = mysqli_fetch_row($result)) {
            foreach ($row as $value) {
                $Rid = $value;
                print("<a href='bill_western.php'>رقم الطلب : $value</a><br><br>");
            }
        }
        ?>
    </div>
