...
if (isset($_POST['upload'])) {
    ...
    $query = "UPDATE resturants SET Poster='$filename' WHERE ID=6";
...
<body>
    ...
    <div>
        <?php
        $database = mysqli_connect("localhost", "root", "12345678", "hubreak2_db");
        $query = "select Poster from resturants where ID=6";
        $result = mysqli_query($database, $query);
        while ($data = mysqli_fetch_assoc($result)) {
            ?>
            <img src="./image/<?php echo $data['filename']; ?>" alt="Western Poster">
            <?php
        }
